...
@dataclass
@register_method
class DRConfig(MethodConfig):
    """
    Config for DR method
    ...
    def loss(
        ...
        old_logprobs: TensorType["gen_nums", "batch_size", "response_size"],
        chosen: List[TensorType["batch_size"]],
        rewards: List[TensorType["batch_size"]],
        masks: TensorType["batch_size", "response_size"],
    ):
        ...
        i_hat = rewards[0] > rewards[1]
        i_t = chosen[0]
        p_diff = torch.exp(torch.sum(logprobs[0],dim=-1) -torch.sum(logprobs[1],dim=-1)) / torch.exp(torch.sum(logprobs[0] + logprobs[1],dim=-1))
        is_diff =  torch.exp(torch.sum(old_logprobs[0],dim=-1) -torch.sum(old_logprobs[1],dim=-1)) / torch.exp(torch.sum(old_logprobs[0] + old_logprobs[1],dim=-1))
        dr = i_hat*p_diff + i_t*is_diff - i_hat*is_diff - self.init_kl_coef.approx_kl.item()
        loss = -dr

        # log quantity of interest
        logratio = torch.sum(logprobs,dim=-1)
        ...
        return loss, {
            ...
            "kl_penty": approx_kl.item(),
            "p_diff_mean": torch.mean(torch.exp(torch.sum(logprobs[0],dim=-1) -torch.sum(logprobs[1],dim=-1))).item
        }
