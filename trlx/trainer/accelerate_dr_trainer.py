...
from trlx.models.modeling_dr import (
...
from trlx.pipeline.dr_pipeline import DRRolloutStorage
...
@register_trainer
class AccelerateDRTrainer(AccelerateRLTrainer):
    """DR Accelerate Trainer"""

    reward_fn: Callable[[List[str], List[str], List[str]], List[float]]
    tokenizer: AutoTokenizer

    def __init__(self, config: TRLConfig, **kwargs):
        """DR Accelerate Trainer initialization
        ...
        self.store = DRRolloutStorage(self.tokenizer.pad_token_id, self.tokenizer.padding_side)
    ...
    def get_arch(self, config: TRLConfig):
        ...
        #     print("Using dr, return Model-wrapper MistralModelWithHydraHead")
    ...
    def loss(self, batch: DRRLBatch):
        ...
        response_length = response_tensors.shape[2]
        ...
        tokens, attention_mask, position_ids, outputs, logits,  logprobs, chosen = [], [], [], [], [], [], []
        ...
        for i in range(num_responses_per_query):
            ...
            logprobs.append(logprobs_of_labels(logits[i][:, :-1, :], tokens[i][:, 1:])[:, start:end])
        ...
        loss, stats = self.config.method.loss(
            ...
            rewards=[rewards[i] for i in range(num_responses_per_query)],
            chosen=[chosen[i] for i in range(num_responses_per_query)],
            masks=[attention_mask[i][:, start + 1 : end + 1] for i in range(num_responses_per_query)],
    ...
    def prepare_learning(self):
        ...
        self.n_inner_epochs = self.config.method.dr_epochs
    ...
    def make_experience(self, num_rollouts: int = 1024, iter_count: int = 0):  # noqa:
        ...
        while len(dr_rl_elements) < num_rollouts:
            stats = {}
            ...
            tbar.set_description(f"[rollout {len(dr_rl_elements)} / {num_rollouts}]")
        ...
        self.push_to_store(dr_rl_elements)
